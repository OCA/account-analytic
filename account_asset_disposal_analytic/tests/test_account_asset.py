...
class TestAccountAsset(TransactionCase):
    ...
    def test_asset_disposal(self):
        ...
        # Disposal move created must have analytic account B in loss move line
        loss = asset.disposal_move_id.line_id.filtered(
            lambda x: x.account_id == self.account_asset_loss)[0]
        self.assertEqual(self.analytic_b.id, loss.analytic_account_id.id)
