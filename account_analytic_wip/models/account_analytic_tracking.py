...
class AnalyticTrackingItem(models.Model):
    ...
    def _get_accounting_data_for_valuation(self):
        ...
        if not accounts.get("stock_journal"):
            exceptions.ValidationError(
                _("Missing Stock Journal for Category {} when closing {}").format(
    ...
    def _create_wip_journal_entry(self):
        ...
        if not wip_journal:
            exceptions.ValidationError(
                _("Missing Stock Journal for Product {} in operation {}").format(
        ...
        if amount and wip_journal:
            ...
            if not acc_wip:
                raise exceptions.ValidationError(
                    _("Missing WIP Account for Product {} in operation {}").format(
                        self.product_id.display_name, self.display_name
                    )
                )
            move_lines = [
