...
class TestAccountAnalyticLineInvoice(TransactionCase):
    def test_account_analytic_line_invoice(self):
        ...
        self.env['account.config.settings'].create({
            'account_analytic_line_invoice_product_id': invoice_product.id,
        }).execute()
        self.assertEqual(
            self.env['account.config.settings'].create({})
            .account_analytic_line_invoice_product_id,
            invoice_product,
        )
        self.env['account.config.settings'].create({
            'account_analytic_line_invoice_product_id': False,
        }).execute()
        self.assertFalse(
            self.env['account.config.settings'].create({})
            .account_analytic_line_invoice_product_id
