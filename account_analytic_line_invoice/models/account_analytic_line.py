...
class AccountAnalyticLine(models.Model):
    ...
    invoice_discount_id = fields.Many2one(
        'account.analytic.line.invoice.discount',
        ondelete='restrict', string='Discount',
        default=lambda self:
        self.env.ref('account_analytic_line_invoice.discount_none', False),
    )
    ...
    @api.multi
    def _account_analytic_line_invoice(self):
        """Create invoices for lines in self"""
        invoices = self.env['account.invoice']
        for lines in self._account_analytic_line_invoice_group_invoice():
            invoices += self.search(
                lines['__domain']
            )._account_analytic_line_invoice_create_invoice()
    ...
    @api.multi
    def _account_analytic_line_invoice_create_invoice(self):
        ...
        for lines in self._account_analytic_line_invoice_group_invoice_lines():
            self.search(
                lines['__domain']
            )._account_analytic_line_invoice_create_invoice_line(invoice)
        invoice.compute_taxes()
        return invoice

    @api.multi
    def _account_analytic_line_invoice_create_invoice_line(self, invoice):
        ...
        product = (
            first.product_id or
            first.account_id.account_analytic_line_invoice_product_id or
            invoice.partner_id.account_analytic_line_invoice_product_id
        )
        if not product:
            raise UserError(_(
                'Neither your line, not the product nor the partner define a '
    ...
    def _account_analytic_line_invoice_group_invoice_lines(self):
    ...
    def _account_analytic_line_invoice_group_invoice(self):
